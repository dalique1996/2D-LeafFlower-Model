...
//DISTANCE CONSTRAINT STIFFNESS
   //Find border points in L1 between domains
   Point3d bordADMD, bordMDAB, bordABPZ;
   for (CCIndex e:ts.edges()){
        std::set<CCIndex> edgeFaces=ts.incidentCells(e,2);
        if(edgeFaces.size()==1)
          continue;
        else if(edgeFaces.size()==2){
           std::vector<CCIndex> edgeFacesV(edgeFaces.begin(), edgeFaces.end());
           if(faceAttr[edgeFacesV[0]].cellLayer==L1 && faceAttr[edgeFacesV[1]].cellLayer==L1){
               if((faceAttr[edgeFacesV[0]].cellZone==MD && faceAttr[edgeFacesV[1]].cellZone==AD) || (faceAttr[edgeFacesV[1]].cellZone==MD && faceAttr[edgeFacesV[0]].cellZone==AD)){
                 std::pair<CCIndex, CCIndex> endADMD=ts.edgeBounds(e);
                 bordADMD=indexAttr[endADMD.first].pos+((indexAttr[endADMD.second].pos-indexAttr[endADMD.first].pos)/2);
               }
               else if((faceAttr[edgeFacesV[0]].cellZone==MD && faceAttr[edgeFacesV[1]].cellZone==AB) || (faceAttr[edgeFacesV[1]].cellZone==MD && faceAttr[edgeFacesV[0]].cellZone==AB)){
                  std::pair<CCIndex, CCIndex> endMDAB=ts.edgeBounds(e);
                  bordMDAB=indexAttr[endMDAB.first].pos+((indexAttr[endMDAB.second].pos-indexAttr[endMDAB.first].pos)/2);
               }
               else if((faceAttr[edgeFacesV[0]].cellZone==AB && faceAttr[edgeFacesV[1]].cellZone==PZ) || (faceAttr[edgeFacesV[1]].cellZone==AB && faceAttr[edgeFacesV[0]].cellZone==PZ)){
                  std::pair<CCIndex, CCIndex> endABPZ=ts.edgeBounds(e);
                  bordABPZ=indexAttr[endABPZ.first].pos+((indexAttr[endABPZ.second].pos-indexAttr[endABPZ.first].pos)/2);
               }
           }
        }
   }

   //Experimental AFM values
   double stiff[3][5] = {{0.56,0.56,0.69,0.25},
                           {0.2,0.34,0.51,1.6},
                           {0.20,0.22,0.20,0.22}};
   for (int f=0 ; f < 3; f++){
       for (int c= 0; c < 5; c++) {
   ...
   int steps[1][5]= {0, FinalStep/3, 2*(FinalStep/3), FinalStep};

   int nAD=0, nMD=0, nAB=0;
   double kAD=0, kMD=0, kAB=0;

   //Stiffness variability inside each domain for L1
   for(CCIndex f: ts.faces()){
       FaceData& fD = faceAttr[f];
       fD.a=0;
           if(fD.cellLayer==L1){
               if(fD.cellZone==AD){
                 if(fD.cellPosition==four)
                    fD.a=1;
                 else if(fD.cellPosition==six)
                     fD.a=1.5;
                 nAD++;
                 kAD+=fD.a;
               }
               else if (fD.cellZone==MD){
                   if(fD.cellPosition==three)
                      fD.a=1;
                   else if(fD.cellPosition==four)
                      fD.a=2.5;
                 nMD++;
                 kMD+=fD.a;
               }
               else if(fD.cellZone==AB){
                  if(fD.cellPosition==one)
                      fD.a=1;
                   else if(fD.cellPosition==two)
                       fD.a=2;
                   else if(fD.cellPosition==three)
                       fD.a=4;
                  nAB++;
                  kAB+=fD.a;
               }
   ...
   for(CCIndex f: ts.faces()){
       ...
       if(fD.cellLayer==L1){
           if(fD.cellZone==AD){
              if(stepCount<=steps[0][1]){
                stiffness=(stepCount*((stiff[0][1]-stiff[0][0])/steps[0][1]))+stiff[0][0];
                stiffness*=nAD;
                stiffness/=kAD;
                stiffness*=fD.a;
              }
              else if(stepCount<=steps[0][2]){
                stiffness=(stepCount*((stiff[0][2]-stiff[0][1])/steps[0][2]))+stiff[0][1];
                stiffness*=nAD;
                stiffness/=kAD;
                stiffness*=fD.a;
              }
              else if(stepCount<=steps[0][3]){
                stiffness=(stepCount*((stiff[0][3]-stiff[0][2])/steps[0][3]))+stiff[0][2];
                stiffness*=nAD;
                stiffness/=kAD;
                stiffness*=fD.a;
              }
              else
                 throw("MechanicalSolver::solveContraints too many steps");
           }
           else if(fD.cellZone==MD){
              if(stepCount<=steps[0][1]){
                stiffness=(stepCount*((stiff[1][1]-stiff[1][0])/steps[0][1]))+stiff[1][0];
                stiffness*=nMD;
                stiffness/=kMD;
                stiffness*=fD.a;
              }
              else if(stepCount<=steps[0][2]){
                stiffness=(stepCount*((stiff[1][2]-stiff[1][1])/steps[0][2]))+stiff[1][1];
                stiffness*=nMD;
                stiffness/=kMD;
                stiffness*=fD.a;
              }
              else if(stepCount<=steps[0][3]){
                stiffness=(stepCount*((stiff[1][3]-stiff[1][2])/steps[0][3]))+stiff[1][2];
                stiffness*=nMD;
                stiffness/=kMD;
                stiffness*=fD.a;
              }
              ...
              if(stepCount<=steps[0][1]){
                stiffness=(stepCount*((stiff[2][1]-stiff[2][0])/steps[0][1]))+stiff[2][0];
                stiffness*=nAB;
                stiffness/=kAB;
                stiffness*=fD.a;
              }
              else if(stepCount<=steps[0][2]){
                stiffness=(stepCount*((stiff[2][2]-stiff[2][1])/steps[0][2]))+stiff[2][1];
                stiffness*=nAB;
                stiffness/=kAB;
                stiffness*=fD.a;
              }
              else if(stepCount<=steps[0][3]){
                stiffness=(stepCount*((stiff[2][3]-stiff[2][2])/steps[0][3]))+stiff[2][2];
                stiffness*=nAB;
                stiffness/=kAB;
                stiffness*=fD.a;
              }
         ...
         else if(fD.cellZone==CZ || fD.cellZone==PZ)
           stiffness=1; //maximal to avoid growth
          else
           throw(QString("Stiffness: Wrong cellZone defined"));
       }
...
//Calculate strain constraint directions
        for(CCIndex f:ts.faces()){
            FaceData& fD = faceAttr[f];
            if (fD.cellLayer==L1){ //L1 follow stress direction
                if(fD.cellZone==MD)
                  fD.stress=Rotate90(fD.force);
                else
                  fD.stress=fD.force;
            }
                ...
                else if(stepCount>35000 && stepCount <=45000){ //X axis
                     Point3d a, b;
                     a=ref_pos-fD.newcentroid;
                     b=Point3d(0,1,0);
                     double angres = mdx::angle(a,b);
                     double relation=(0.0+stepCount-35000)/(45000-35000);
                     Point3d out;

                     if(a[0]<0)
                       out = RotateClock(a, angres*relation);
                     else
                       out = RotateCounterClock(a, angres*relation);
                     fD.stress=Rotate90(out);
                }
                 else
                  fD.stress=Rotate90(Point3d(0,1,0));
            }
              ...
              double stiffnessXY=0;
                  ...
                  if(fD.cellZone==AD){
                    if(fD.cellLayer==L1)
                      ...
                      if(fD.cellLayer==L1)
                          stiffnessXX= (0.16*(stepCount/FinalStep))+0.7;
                      else
                          stiffnessXX=(0.24*(stepCount/FinalStep))+0.7;
                  }
                  else if(fD.cellZone==MD){
                      if(fD.cellLayer==L1)
                          stiffnessXX=0;
                      else
                          stiffnessXX= (0.16*(stepCount/(FinalStep)))+0.6;
                  }
